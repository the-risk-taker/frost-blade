...
import { ITEMS, give } from './items.js'
...
export function devClick(game, element) {
    ...
    if (item) give(p, item, { gold: 100 }[item] ?? (ITEMS[item].slot ? 1 : 10))
...
// Outlines as [x, y, w, h]. The fight code checks distances, so these show roughly what it tests: bodies, the sword swing and the whirl.
export function hitboxes(game) {
    ...
    if (p.attackT >= 0) boxes.push([p.x + p.dir * 16 - 30, p.y - 34, 60, 34])
