...
export const SLOTS = ['sword', 'frost', 'potion']
...
export function createPlayer() {
  return {
    ...
    slot: 0, potions: 3, cooldown: 0,
    attackT: -1, hitSet: new Set(), rollT: -1, hurtT: 0,
  }
}

export function canUse(p, item) {
  return { sword: p.stamina >= 12, frost: p.mana >= 25, potion: p.potions > 0 }[item]
}
...
function useItem(p, game) {
  ...
  if (!item || !canUse(p, item) || p.cooldown > 0 || p.attackT >= 0) return
  if (item === 'sword') {
    p.stamina -= 12
    p.restT = 0.6
    p.attackT = 0
...
export function updatePlayer(p, dt, game) {
  ...
  p.cooldown -= dt
  p.restT -= dt
  if (p.restT <= 0) p.stamina = Math.min(100, p.stamina + 30 * dt)
  p.mana = Math.min(100, p.mana + 6 * dt)
  ...
  if (p.rollT >= 0) {
    ...
    const target = stunned || (p.attackT >= 0 && p.onGround) ? 0 : move * SPEED
    ...
    if (!stunned) {
      ...
      if (input.hit('KeyJ', 'Mouse0')) useItem(p, game)
    }
  }
